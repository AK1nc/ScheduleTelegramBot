...
namespace Dep406Bot.Buttons.Indificators
{
    ...
    /// </summary>
    [Description("BachelorStudent")]
    internal class BachelorStudentBT(IChatHistory _db) : IBotCallbackQuery
    {
        ...
        public async Task Realization(ITelegramBotClient client, Update update, CancellationToken cancellationToken)
        {
            try
            {
                await _db.setIndificator(update.CallbackQuery.Message.Chat.Id, 1);
            }
            catch (Exception ex)
            {
